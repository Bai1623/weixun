...
暮调是一个私人调酒手账 Web/PWA。核心用途是：浏览酒谱、根据酒柜筛选能做的酒、每日随机推荐一杯、学习调酒基础、记录每天调过的作品，并把账号级数据备份到腾讯云 CloudBase。

## 当前版本

- 当前代码仓库：`git@github.com:Bai1623/weixun-Twilight-Mixbook.git`（旧 `weixun.git` 会跳转到此仓库）
- 当前开发分支：`codex/twilight-cocktail-prototype`
- 最近确认的功能源码 commit：`b443af9`
- 当前 Pages 仓库：`git@github.com:Bai1623/weixun-Twilight-Mixbook.git`
- 当前 Pages 分支：`gh-pages`
- 最近确认的 Pages 分支 commit：`5aead54`
- 线上地址：[https://bai1623.github.io/weixun-Twilight-Mixbook/](https://bai1623.github.io/weixun-Twilight-Mixbook/)
- 带缓存刷新参数的账号与数据页：[https://bai1623.github.io/weixun-Twilight-Mixbook/?v=b443af9#/profile/data](https://bai1623.github.io/weixun-Twilight-Mixbook/?v=b443af9#/profile/data)
...
- 我的作品分享：支持长图导出和筛选导出；JSON 导入导出已统一放到“我的 / 账号与数据”。
- 朋友想喝：生成分享链接后，朋友可提交一条无照片点单；列表默认折叠，可展开、删除；关闭链接会在确认后清空该链接下全部点单。
- 账号与数据：账号登录/切换、云端检查、备份、恢复、撤销恢复、自动备份、JSON 导入导出和清除本机数据集中管理。
- 跨设备备份：账号级轻量增量元数据保存在 CloudBase；作品手机原图和 1280px 预览图保存在私有阿里云 OSS。
...
- 用户在“我的 / 账号与数据”里输入云端账号和密码，这是一套应用自己的轻量账号逻辑，不是腾讯云账号。
- 登录/创建账号、备份、恢复、朋友点单都通过 `/share` HTTP 云函数路由。
- 登录或切换账号会先读取目标账号摘要并请求确认；确认后以云端为准，整包替换本机的作品和照片、酒柜、收藏、课程进度、每日推荐、自定义材料及自动备份设置。取消时本机数据不变。
- “账号与数据”在登录状态下每次打开都会调用只读 `account-summary` 查询云端备份，也可手动点击“检查云端备份”；页面分别展示备份时间、作品数、预览图/原图覆盖量和其他账号数据数量。“最近操作”与云端实际状态分开展示。
...
- 每次手动或自动备份前都会重新查询云端版本。发现另一台设备已更新，或无法确认本机基于哪个云端版本时，会阻止备份，避免静默覆盖远端数据；云函数写入时还会在数据库事务内再次比较预期版本，防止“查询后、写入前”另一台设备抢先更新。
- 最新版本备份使用 `metadata-patch`：第一次备份全部轻量元数据，之后只备份新增/编辑/删除过的作品元数据。
- 照片二进制不会进入轻量备份包。数据库只保存 OSS 对象键；浏览器使用云函数签发的短期 PUT/GET URL 直传 OSS。
- 新增照片先写入本机 IndexedDB，再由一次统一云端备份上传；照片签名和轻量元数据遇到临时网络错误会重试一次。云端仍失败时作品保留在本机并提示稍后手动备份。
- 照片上传队列会在刷新页面、切换账号、恢复和撤销恢复后重新读取 IndexedDB；页面按作品展示等待上传/上传失败状态和具体原因，可只重试该作品，不会顺带上传其他失败项。
- 完整恢复以云端照片元数据为准：只有照片版本和对象键完全一致时才复用本机缓存；云端已换图或删图时不会继续显示旧本地照片。
- 新电脑无本地作品时，登录后会自动恢复账号数据，并默认下载全部作品预览图；支持进度、暂停、继续，并按作品展示下载失败原因和单项重试。
- 手机原图不会批量自动下载，在作品卡片上按需下载。
- IndexedDB 保存本地照片 Blob；带云端照片版本的记录不会再把 Base64 图片写入 localStorage。
- 旧版 Base64 照片会在下次云端备份时迁移为“仅预览”备份，无法补回原始手机文件。
...
2026-08-23 P2 账号与数据体验版本验证结果：前端 `19` 个测试文件、`141` 个测试通过；TypeScript 类型检查、ESLint 和 GitHub Pages 构建均通过；桌面端与 390px 移动端无横向溢出；线上资源已确认包含“账号与数据”“检查云端备份”“备份到云端”“从云端恢复”等新版界面。云函数本次未修改，无需重新部署。
...
7. 部署后打开“我的 / 账号与数据”，登录云端账号，新增带照片作品并测试备份、跨浏览器自动恢复预览和按需下载原图。
...
必须先部署云函数、再部署对应前端。新版前端会严格检查 `account-summary` 返回契约，旧云函数的未知成功响应不会被误判为空备份。如果前端提示“云函数不支持云端备份摘要”“云函数不支持增量备份”或“无法连接云函数”，优先检查：
...
- 推荐在旧电脑先打开“我的 / 账号与数据”，登录云端账号，点击“备份到云端”。
