<template>
  <div v-if="cocktail" class="grid gap-8 md:grid-cols-[0.9fr_1.1fr]">
    ...
    <div>
      ...
      <p class="ui-panel mt-6 p-4 text-sm leading-6 text-cream">
        推荐理由：{{ daily.reason || '经典热门酒款，制作步骤少，适合作为今日练习。' }}
      </p>
      <div class="mt-6 flex flex-col gap-3 sm:flex-row">
        <RouterLink class="ui-button-primary text-center" :to="`/cocktails/${cocktail.slug}`">
          查看配方
        </RouterLink>
        <RouterLink
          class="ui-button-secondary text-center text-gold"
